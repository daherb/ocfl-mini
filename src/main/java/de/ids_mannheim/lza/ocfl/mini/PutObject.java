...
public class PutObject extends Action {
    ...
    private void createObject(Storage storage, String id, File path) throws StorageException {
        ...
        Inventory versionInventory = new Inventory(
                "content" // contentDirectory
                , storage.getDigestAlgorithm().toString() // digestAlgorithm
                , "v1" // head
                , id // id
        ...
        try {
            ...
            om.writeValue(versionInventoryFile, versionInventory);
            om.writeValue(objectInventoryFile, objectInventory);
            String versionHash = storage.getDigestAlgorithm().hashFile(versionInventoryFile);
            String objectHash = storage.getDigestAlgorithm().hashFile(objectInventoryFile);
            try (FileOutputStream fos = new FileOutputStream(
                    Path.of(versionFolder.getPath(),"inventory.json.sha512").toFile())) {
                    fos.write((versionHash + "\tinventory.json").getBytes());
            }
            try (FileOutputStream fos = new FileOutputStream(
                    Path.of(tmpFolder.getPath(),"inventory.json.sha512").toFile())) {
                    fos.write((objectHash + "\tinventory.json").getBytes());
            }
        }
        catch (IOException e) {
